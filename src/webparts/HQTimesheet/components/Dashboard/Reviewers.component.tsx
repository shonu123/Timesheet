                        ...
                        PTOTransactionRecords.forEach(pto => {
                         ...
                         // Handling If an employee submits a time-off range within the previous year and the submission date is in the current year
                                    ...
                                    if (PTOTransactionRecords.length) { // if existing record found consider the existing Submitted date
                                        SubmittedDate = new Date(DateUtilities.GetDateMMDDYYYYAsInList(PTOTransactionRecords[0].SubmittedDate));
                                    }
                                    if (new Date(InitialRecord[0].Date).getFullYear() == (new Date(SubmittedDate).getFullYear() - 1)) {
                                        await this.updateEmployeePTOAndOpeningPTOTnForCurrentYear(TimeOffPostData['Status'],InitialRecord[0].EmployeeId,PTOHrs);
                                    }
                    }
                    sp.web.lists.getByTitle('TimeOffEmployees').items.getById(TimeOffRecordID).inBatch(PTOTransactionBatch).update(TimeOffPostData);//TimeOff update
