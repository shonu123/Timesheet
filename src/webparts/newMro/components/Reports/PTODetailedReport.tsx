...
class PTODetailedReport extends React.Component<PTODetailedReportProps, PTODetailedReportState> {
    ...
    private async getClientEmployees(value) {
        try {
            if (value != "All Clients") {
                ...
                if (EmpObj.length > 0) {
                    // EmpObj.unshift({ ID: "0", Title: "All Employees" });
                    this.setState({ EmployeesObj: EmpObj, loading: false, isHavingEmployees: true, EmployeeId: [] });
                }
                else {
                    this.setState({ EmployeesObj: EmpObj, loading: false, isHavingEmployees: false, EmployeeId: [] });
                    customToaster('toster-error', ToasterTypes.Error, 'There are no employees associated with this client', 4000);
                }
            }
            else {
                this.setState({ EmployeesObj: this.state.AllEmployees, loading: false, isHavingEmployees: true, EmployeeId: [] })
            }
        }
        catch (error) {
            customToaster('toster-error', ToasterTypes.Error, 'Sorry! something went wrong', 4000);
    ...
    private handleChangeEvents = (event, actionMeta?) => {
        ...
        if (![null, undefined].includes(event) && event.target != undefined) {
            ...
            if (name == 'EmployeeId' && inputvalue[inputvalue.length - 1] === "all") {
                value = this.state.EmployeeId.length === this.state.EmployeesObj.length ? [] : this.state.EmployeesObj.map(emp => emp.ID);
            }

        }
        else if (actionMeta != undefined) {
            name = actionMeta.name;
            value = actionMeta.action == 'clear' ? name == 'EmployeeId' ? [] : '' : event.value;
        }
        this.setState({ [name]: value, PTOData: [], PTOExcelData: [] });
    }
    ...
    private handleCancel = async (e) => {
        ...
        this.setState({ ClientName: "All Clients", EmployeeId: [], startDate: null, endDate: null, GenerateonlyGranted: false, EmployeesObj: this.state.AllEmployees, PTOData: [], PTOExcelData: [] });
    ...
    private handleSubmit = () => {
        ...
        let data = {
            ...
            Employee: { val: this.state.EmployeeId, required: true, Name: 'Employee', Type: ControlType.MUIMultiSelect, Focusid: 'Employee' },
        }
    ...
    private generateDatatable = async (postObject) => {
        ...
        try {
            ...
            if (Employee.length != this.state.EmployeesObj.length) {
                reportData = reportData.filter(report => Employee.includes(report.Employee.Id));
            }
            if (reportData.length > 0) {
                ...
                customToaster('toster-error', ToasterTypes.Error, 'No records found!', 4000);
                this.setState({ loading: false })
            }
        }
        catch (error) {
