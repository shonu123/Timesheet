...
class Units extends React.Component<UnitsProps, UnitsState>{
  ...
  private onSucess = () => {
    this.setState({ modalTitle: 'Success', modalText: 'Quantity for Unit submitted successfully', showHideModal: true, loading: false, isSuccess: true, ItemID: 0, Unit: "",Description:"", errorMessage: "" });
  }

  private onUpdateSucess = () => {
    this.setState({ modalTitle: 'Success', modalText: 'Quantity for Unit updated successfully', showHideModal: true, loading: false, isSuccess: true, ItemID: 0, Unit: "", Description:"",errorMessage: "" });
  }
  ...
  private onEditClickHandler = (id) => {
    ...
    try {
      sp.web.lists.getByTitle('Units').items.getById(id).get()
        .then((response) => {
          this.setState({
            addNewUnit: true,
  ...
  public submitImportedExcelData = () => {
    ...
    for (var i = excelData.length - 1; i >= 0; i--) {
      for (var j = 0; j < BuyersData.length; j++) {
        if (excelData[i] && (excelData[i].Unit.toLowerCase() === BuyersData[j].Title.toLowerCase())) {
  ...
  public updateImportExceldata = async (nonDuplicateRec, statusChangedData) => {
    statusChangedData.forEach(element => {
        sp.web.lists.getByTitle('Units').items.getById(element.Id).update(element).then((res) => {
  ...
  public insertImportedExcelData = async (data) => {
    try {
      this.setState({ loading: true });
      let list = await sp.web.lists.getByTitle("Units");
      ...
      await batch.execute()
        .then((res) => {
          ...
          this.setState({
            modalTitle: 'Success',
            modalText: 'Units uploaded successfully',
            showHideModal: true,
            isSuccess: true,
          });
          this.resetImportField();
  ...
  public ErrorFileSelect = () => {
    ...
    this.setState({
      loading: false,
      modalTitle: 'Alert',
      modalText: 'Invalid Quantity for Units file selected',
      showHideModal: true,
      isSuccess: false
    });
  }
          ...
          return (
            <React.Fragment>
              <div style={{ paddingLeft: '10px' }}>
                <NavLink title="Edit" className="csrLink ms-draggable" to={`/units/${record.Id}`}>
                  <FontAwesomeIcon icon={faEdit} onClick={() => { this.onEditClickHandler(record.Id); }}/>
                </NavLink>
              </div>
            </React.Fragment>
          );
