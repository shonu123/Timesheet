...
class Programs extends React.Component<ProgramsProps, ProgramsState>{
  ...
  constructor(props) {
    ...
    this.ProgramDescription = React.createRef();
  }

  public componentDidMount() {
    //console.log('Project Code:', this.props);
    highlightCurrentNav("Programs");
    this.GetOnloadData();
  }
  public componentWillReceiveProps(newProps) {
    if (newProps.match.params.id == undefined)
      this.setState({ Program: '',Description:'', IsActive: true, SaveUpdateText: 'Submit', addNewProgram: false });
  }
  ...
  private addNew = () => {
    this.setState({ addNewProgram: true });
  }
  ...
  private SunmitData = () => {
    let data = {
      Program: { val: this.state.Program, required: true, Name: 'Program', Type: ControlType.string, Focusid: this.inputProgram },
      Description: { val: this.state.Description, required: false, Name: 'Description', Type: ControlType.string, Focusid: this.ProgramDescription },
    };
    let isValid = formValidation.checkValidations(data);
    var formdata = {
      Title: this.state.Program,
  ...
  private onSucess = () => {
    this.setState({ modalTitle: 'Success', modalText: 'Program submitted successfully', showHideModal: true, loading: false, isSuccess: true, ItemID: 0, Program: "", Description:"", errorMessage: "" });
  }

  private onUpdateSucess = () => {
    this.setState({ modalTitle: 'Success', modalText: 'Program updated successfully', showHideModal: true, loading: false, isSuccess: true, ItemID: 0, Program: "", Description:"", errorMessage: "" });
  }
  ...
  private onEditClickHandler = (id) => {
    ...
    try {
      sp.web.lists.getByTitle('Programs').items.getById(id).get()
        .then((response) => {
          this.setState({
            addNewProgram: true,
            Program: response.Title.trim(),
  ...
  public submitImportedExcelData = () => {
    ...
    for (var i = excelData.length - 1; i >= 0; i--) {
      for (var j = 0; j < BuyersData.length; j++) {
        if (excelData[i] && (excelData[i].Program.toLowerCase() === BuyersData[j].Title.toLowerCase())) {
          if (excelData[i].Status === BuyersData[j].Status) {
    ...
    if (excelData.length) {
      excelData.forEach(item => {
        var obj = {};
        obj["Title"] = item.Program.toString();
  ...
  public updateImportExceldata = async (nonDuplicateRec, statusChangedData) => {
    statusChangedData.forEach(element => {
        sp.web.lists.getByTitle('Programs').items.getById(element.Id).update(element).then((res) => {
            ...
            if (!nonDuplicateRec.length) {
                ...
                this.setState({
                    modalTitle: 'Success',
                    modalText: 'Program updated successfully',
                    showHideModal: true,
                    isSuccess: true
                });
                this.resetImportField();
  ...
  public insertImportedExcelData = async (data) => {
    try {
      this.setState({ loading: true });
      let list = await sp.web.lists.getByTitle("Programs");
      ...
      await batch.execute()
        .then((res) => {
          ...
          this.setState({
            modalTitle: 'Success',
            modalText: 'Programs uploaded successfully',
            showHideModal: true,
            isSuccess: true,
          });
          this.resetImportField();
          ...
          return (
            <React.Fragment>
              <div style={{ paddingLeft: '10px' }}>
                <NavLink title="Edit" className="csrLink ms-draggable" to={`/Programs/${record.Id}`}>
              ...
              <div className="col-12 col-md-12 col-lg-7">
                ...
                <div className={this.state.addNewProgram ? 'mx-2 activediv' : 'mx-2'}>
                  <div className="text-right pt-2" id="">
                    <ImportExcel ErrorFileSelect={this.ErrorFileSelect} columns={["Program","Description", "Status"]} filename="Programs" onDataFetch={this.fetchImportedExcelData} submitData={this.submitImportedExcelData}></ImportExcel>
                      ...
                      <div className="row">
                        ...
                        <div className="col-md-4">
                            <div className="light-text">
                                ...
                                <textarea rows={2} className="form-control" ref={this.ProgramDescription} maxLength={1000} placeholder="" name="Description" title="Description" value={this.state.Description || ''} autoComplete="false" onChange={this.handleChange}></textarea>
                            </div>
                        </div>
                        <InputCheckBox
                          label={"Is Active"}
                ...
                <div className="light-box border-box-shadow mx-2 table-head-1st-td">
                  <TableGenerator columns={columns} data={this.state.data} fileName={'Programs'} showExportExcel={true} ExportExcelCustomisedColumns={ExportExcelreportColumns}></TableGenerator>
                </div>
              </div>
            </div>
          </div>
